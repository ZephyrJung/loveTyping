...
states["menu"] = {
    ...
    onEnter = function(self)
        ...
        self.buttons = {}
           ...
           -- Responsive dimensions (scale against design baseline of 960x700)
        local baseScale = screenScale(1)
        local btnW = math.min(w * 0.82, 340 * baseScale)
        local btnH = math.floor(68 * baseScale)

           -- Button vertical positions: centered on portrait, top-aligned on landscape
        local startY
        if portrait then
            local totalH = 3 * btnH + 2 * 36 * baseScale
            startY = (h - totalH) * 0.38
        else
            startY = math.floor(h * 0.35)
        end

           -- Build button rects for each level
        for i = 1, 3 do
            table.insert(self.buttons, {x = (w - btnW) / 2,
                y = startY + (i - 1) * (btnH + 36 * baseScale),
                w = btnW, h = btnH, id = i})
        end
    end,
    ...
    onUpdate = function(self, dt)
        ...
        if self.mouseClicked then
            ...
            local mmx = self.clickX or 0
            local mmy = self.clickY or 0
            for i, btn in ipairs(self.buttons) do
                local bx, by, bw, bh = btn.x, btn.y, btn.w, btn.h
                if type(bx) == "number" and type(by) == "number" and
                   type(bw) == "number" and type(bh) == "number" then
                    if (mmx >= bx and mmx <= bx + bw and
                        mmy >= by and mmy <= by + bh) then
    ...
    onDraw = function(self)
        ...
        local titleFont = love.graphics.newFont(math.floor(48 * baseScale))
        love.graphics.setFont(titleFont)
        love.graphics.setColor(1, 1, 1, 1)
        love.graphics.printf("loveTyping", w / 2, math.floor(h * 0.15),
                             w * 0.7, "center")

           -- Subtitle
        local subFont = love.graphics.newFont(math.floor(14 * baseScale))
        love.graphics.setFont(subFont)
        love.graphics.setColor(0.50, 0.65, 0.85, 0.7)
        love.graphics.printf("Select Difficulty Level", w / 2, math.floor(h * 0.23),
                             w * 0.5, "center")
        ...
        for i, btn in ipairs(self.buttons) do
            ...
            local txtFont = love.graphics.newFont(math.floor(20 * baseScale))
            ...
            local descFont = love.graphics.newFont(math.floor(12 * baseScale))
        ...
        local hintFont = love.graphics.newFont(math.floor(13 * baseScale))
        love.graphics.setFont(hintFont)
        love.graphics.setColor(0.35, 0.40, 0.50, 0.55)
        love.graphics.printf("Press [1] [2] or [3] to quick-select", w / 2, h - 38 * baseScale,
                             w * 0.45, "center")
    end,
