...
-- Each step on the road is a character — press it to advance.
    ...
    if blds then
        ...
        -- Draw the road: characters only, no lines or decorations.
          ...
          for i = 1, #waypoints do
              ...
              if i == thiefIdx then
                    -- Current target: bright green characters with pulsing glow.
                  ...
                  love.graphics.setLineWidth(1)
                  love.graphics.setColor(0.10, 0.60, 0.15, 1.0)
              elseif i < thiefIdx then
                    -- Already passed: dim gray characters (faded).
                  love.graphics.setLineWidth(1 * sc)
                  love.graphics.setColor(0.30, 0.32, 0.36, 0.5)
                  love.graphics.setLineWidth(1)
              else
                    -- Upcoming: medium gray characters (normal visibility).
                  love.graphics.setLineWidth(1 * sc)
                  love.graphics.setColor(0.40, 0.42, 0.48, 0.75)
                  love.graphics.setLineWidth(1)
              end

                    -- Draw the character: clear readable size (not too big, not too small).
